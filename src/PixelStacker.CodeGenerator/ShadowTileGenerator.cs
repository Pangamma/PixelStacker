...
namespace PixelStacker.CodeGenerator
{
    ...
    public class ShadowTileGenerator
    {
        ...
        {
            string rootShadowsPath = @"D:\git\PixelStacker\src\PixelStacker.Resources\Images\Shadows";

            {
                string fToStrip = $@"{rootShadowsPath}\sprites\sprite-source-x{depth}.png";
                var saveFile = $@"{rootShadowsPath}\sprites\sprite-x{depth}.png";

                using (SKBitmap _img = SKBitmap.Decode(fToStrip))
                {
                    using var img = _img.Copy(SKColorType.Rgba8888);
                    var imgFixed = img.ToEditStream(null, (x, y, c) =>
                    {
                        ...
                        return new SKColor(0, 0, 0, (byte)nAlpha);
                    });

                    using var ms = new MemoryStream();
                    bool isSuccessfulEncode = imgFixed.Encode(ms, SKEncodedImageFormat.Png, 100);
