...
namespace PixelStacker.Core.Engine.Quantizer.ColorCaches.Octree
{
    public class OctreeColorCache : BaseColorCache
    {
        ...
        /// </summary>
        protected override void OnCachePalette(IList<PxColor> palette)
        {
            int index = 0;

            foreach (PxColor color in palette)
            {
        ...
        /// </summary>
        protected override void OnGetColorPaletteIndex(PxColor color, out int paletteIndex)
        {
            Dictionary<int, PxColor> candidates = root.GetPaletteIndex(color, 0);
