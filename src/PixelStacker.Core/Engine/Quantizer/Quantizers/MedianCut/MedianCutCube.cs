...
namespace PixelStacker.Core.Engine.Quantizer.Quantizers.MedianCut
{
    internal class MedianCutCube
    {
        ...
        public PxColor Color
        {
            get
            {
                ...
                {
                    PxColor color = PxColor.FromArgb(argb);
                ...
                PxColor result = PxColor.FromArgb(255, red, green, blue);
                return result;
            }
        }
        ...
        public void SplitAtMedian(byte componentIndex, out MedianCutCube firstMedianCutCube, out MedianCutCube secondMedianCutCube)
        {
            ...
            switch (componentIndex)
            {
                // red colors
                case 0:
                    colors = colorList.OrderBy(argb => ColorModelHelper.GetComponentA(ColorModel, PxColor.FromArgb(argb))).ToList();
                    break;

                // green colors
                case 1:
                    colors = colorList.OrderBy(argb => ColorModelHelper.GetComponentB(ColorModel, PxColor.FromArgb(argb))).ToList();
                    break;

                // blue colors
                case 2:
                    colors = colorList.OrderBy(argb => ColorModelHelper.GetComponentC(ColorModel, PxColor.FromArgb(argb))).ToList();
                    break;
        ...
        public bool IsColorIn(PxColor color)
        {
            int red = ColorModelHelper.GetComponentA(ColorModel, color);
            int green = ColorModelHelper.GetComponentB(ColorModel, color);
            int blue = ColorModelHelper.GetComponentC(ColorModel, color);
