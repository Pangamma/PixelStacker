...
namespace PixelStacker.Core.Engine.Quantizer.Quantizers
{
    public abstract class BaseColorCacheQuantizer : BaseColorQuantizer
    {
        ...
        /// </summary>
        protected abstract List<PxColor> OnGetPaletteToCache(int colorCount);
        ...
        /// </summary>
        protected sealed override List<PxColor> OnGetPalette(int colorCount)
        {
            // use optimization, or calculate new palette if color count is lower than unique color count
            List<PxColor> palette = base.OnGetPalette(colorCount) ?? OnGetPaletteToCache(colorCount);
            GetColorCache().CachePalette(palette);
            return palette;
        }
