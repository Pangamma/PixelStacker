...
namespace PixelStacker.Core.Engine.Quantizer.Quantizers
{
    public abstract class BaseColorQuantizer : IColorQuantizer
    {
        ...
        /// </summary>
        protected virtual void OnAddColor(PxColor color, int key, int x, int y)
        {
        ...
        /// </summary>
        protected virtual List<PxColor> OnGetPalette(int colorCount)
        {
            ...
            if (UniqueColors.Count > 0 && colorCount >= UniqueColors.Count)
            {
                ...
                return UniqueColors.
                    ...
                    Select(pair => PxColor.FromArgb(pair.Key)).
                    Select(color => PxColor.FromArgb(255, color.R, color.G, color.B)).
                    ToList();
            }
        ...
        /// </summary>
        protected virtual void OnGetPaletteIndex(PxColor color, int key, int x, int y, out int paletteIndex)
        {
        ...
        /// </summary>
        public IList<PxPoint> GetPointPath(int width, int heigth)
        {
        ...
        /// </summary>
        public void AddColor(PxColor color, int x, int y)
        {
        ...
        /// </summary>
        public List<PxColor> GetPalette(int colorCount)
        {
        ...
        /// </summary>
        public int GetPaletteIndex(PxColor color, int x, int y)
        {
            color = QuantizationHelper.ConvertAlpha(color, out int key);
