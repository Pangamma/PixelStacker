...
namespace PixelStacker.Core.Engine.Quantizer.Quantizers.Uniform
{
    /// <summary>
    ...
    public class UniformQuantizer : BaseColorQuantizer
    {
        ...
        /// </summary>
        protected override void OnAddColor(PxColor color, int key, int x, int y)
        {
        ...
        /// </summary>
        protected override List<PxColor> OnGetPalette(int colorCount)
        {
            // use optimized palette, if any
            List<PxColor> optimizedPalette = base.OnGetPalette(colorCount);
            if (optimizedPalette != null) return optimizedPalette;

            // otherwise synthetize one
            List<PxColor> result = new List<PxColor>();
            ...
            foreach (UniformColorSlot redSlot in redSlots)
                foreach (UniformColorSlot greenSlot in greenSlots)
                    foreach (UniformColorSlot blueSlot in blueSlots)
                    {
                        ...
                        PxColor color = PxColor.FromArgb(255, red, green, blue);
                        result.Add(color);
                    }
        ...
        /// </summary>
        protected override void OnGetPaletteIndex(PxColor color, int key, int x, int y, out int paletteIndex)
        {
            int redIndex = color.R >> 5;
            int greenIndex = color.G >> 5;
            int blueIndex = color.B >> 6;
        ...
        /// </summary>
        public override string Label => "Uniform quantization";
