...
namespace PixelStacker.Core.Collections.ColorMapper
{
    internal class FloodFillNode
    {
        ...
        public FloodFillNode(int PaletteID, MaterialCombination mc, PxColor c)
        {
            this.PaletteID = PaletteID;
            Value = mc;
            R = c.R;
        ...
        {
            return new FloodFillNode(PaletteID, Value, R + r, G + g, B + b);
        }
    }
    ...
    public class FloodFillMapper : IColorMapper
    {
        ...
        */
        ...
        private bool TrySet(FloodFillNode node)
        {
            ...
            // BATTLE TIME
            PxColor c = PxColor.FromArgb(255, node.R, node.G, node.B);
            long distExisting = Palette[existing.Value].GetAverageColorDistance(IsSideView, c);
            long distNew = node.Value.GetAverageColorDistance(IsSideView, c);
        ...
        public MaterialCombination FindBestMatch(PxColor c)
        {
            ...
            PxColor found = foundMc.GetAverageColor(this.IsSideView);
