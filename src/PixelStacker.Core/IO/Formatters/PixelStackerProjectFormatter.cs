            ...
            try
            {
                ...
                using (FileStream zipToOpen = new FileStream(filePath, FileMode.OpenOrCreate))
                {
                    using (ZipArchive archive = new ZipArchive(zipToOpen, ZipArchiveMode.Update))
                    {
                        ...
                        {
                            PxBitmap bm = canvas.CanvasData.ToBitmap(worker);
                        ...
                        {
                            PxBitmap bm = canvas.PreprocessedImage;
                ...
                using (FileStream zipToOpen = new FileStream(filePath, FileMode.Open))
                {
                    using (ZipArchive archive = new ZipArchive(zipToOpen, ZipArchiveMode.Read))
                    {
...

                        {
                            ...
                            using (StreamReader reader = new StreamReader(entry.Open()))
                            {
                                var bm = PxBitmap.FromStream(reader.BaseStream);
                                canvas.CanvasData = CanvasData.FromBitmap(canvas.MaterialPalette, bm, worker);
                            }
                        }

                        {
                            ...
                            using (StreamReader reader = new StreamReader(entry.Open()))
                            {
                                PxBitmap bm = PxBitmap.FromStream(reader.BaseStream);
...

                        {
                            ...
                            {
                                string json = await reader.ReadToEndAsync();
                                canvas.WorldEditOrigin = JsonConvert.DeserializeObject<PxPoint>(json);
                            }
                        }
