...
// the internal workings of this single class (ColorTreeNodeCollection). The original
...
namespace PixelStacker.Logic.Collections
{
    ...
    /// 
    [Serializable]
    public class ColorTreeNodeCollection<T> : ICollection<NodeDistance<ColorTreeNode<T>>>
    {
        double[] distances;
        ColorTreeNode<T>[] positions;
        ...
        /// </summary>
        /// 
        public ColorTreeNode<T> Farthest
        {
            get
            {
                if (count == 0)
        ...
        /// </summary>
        /// 
        public ColorTreeNode<T> Nearest
        {
            get
            {
                if (count == 0)
        ...
        /// <summary>
        ///   Creates a new <see cref="ColorTreeNodeCollection&lt;T&gt;"/> with a maximum size.
        /// </summary>
        /// 
        /// <param name="size">The maximum number of elements allowed in this collection.</param>
        /// 
        public ColorTreeNodeCollection(int size)
        {
            if (size <= 0)
            ...
            positions = new ColorTreeNode<T>[size];
        }
        ...
        /// 
        public bool Add(ColorTreeNode<T> value, double distance)
        {
        ...
        /// <returns>Returns true if the node has been added; false otherwise.</returns>
        /// 
        public bool AddFarthest(ColorTreeNode<T> value, double distance)
        {
            // The list does have a limit. We have to check if the list
            // is already full or not, to see if we can discard or keep
            // the point
        ...
        /// 
        private void add(double distance, ColorTreeNode<T> item)
        {
        ...
        /// </summary>
        /// 
        public NodeDistance<ColorTreeNode<T>> this[int index]
        {
            get { return new NodeDistance<ColorTreeNode<T>>(positions[index], distances[index]); }
        }
        ...
        /// </returns>
        /// 
        public IEnumerator<NodeDistance<ColorTreeNode<T>>> GetEnumerator()
        {
            for (int i = 0; i < count; i++)
                yield return new NodeDistance<ColorTreeNode<T>>(positions[i], distances[i]);
        ...
        /// 
        public void CopyTo(NodeDistance<ColorTreeNode<T>>[] array, int arrayIndex)
        {
