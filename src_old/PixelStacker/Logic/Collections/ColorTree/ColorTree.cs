namespace PixelStacker.Logic.Collections
{
    ...
    /// </summary>
    /// 
    /// <typeparam name="ColorTreeNodePS<T>">The class type for the nodes of the tree.</typeparam>
    /// 
    [Serializable]
    public class ColorTree<T> : IEnumerable<ColorTreeNode<T>>
    {
        /// <summary>
        ///   Gets the root node of this tree.
        /// </summary>
        /// 
        public ColorTreeNode<T> Root { get; set; }
        private int count;
        ...
        /// <summary>
        ///   Creates a new <see cref="ColorTree&lt;T&gt;"/>.
        /// </summary>
        ///
        /// <param name="dimensions">The number of dimensions in the tree.</param>
        ///
        public ColorTree(int dimensions)
        {
        ...
        ///
        public ColorTree(int dimension, ColorTreeNode<T> Root)
            : this(dimension)
        {
            this.Root = Root;
        ...
        ///
        public ColorTree(int dimension, ColorTreeNode<T> Root, int count, int leaves)
            : this(dimension)
        {
            this.Root = Root;
            this.count = count;
        ...
        ///
        public ICollection<NodeDistance<ColorTreeNode<T>>> Nearest(double[] position, double radius, int maximum)
        {
            if (maximum == 0)
            {
                var list = new List<NodeDistance<ColorTreeNode<T>>>();
            ...
            {
                var list = new ColorTreeNodeCollection<T>(maximum);
        ...
        /// <returns>A list of neighbor points, ordered by distance.</returns>
        ///
        public List<NodeDistance<ColorTreeNode<T>>> Nearest(double[] position, double radius)
        {
            var list = new List<NodeDistance<ColorTreeNode<T>>>();
        ...
        /// <returns>A list of neighbor points, ordered by distance.</returns>
        ///
        public ColorTreeNodeCollection<T> Nearest(double[] position, int neighbors)
        {
            var list = new ColorTreeNodeCollection<T>(size: neighbors);
        ...
        /// <returns>A list of neighbor points, ordered by distance.</returns>
        ///
        public ColorTreeNode<T> Nearest(double[] position)
        {
            double distance;
        ...
        /// <returns>A list of neighbor points, ordered by distance.</returns>
        ///
        public ColorTreeNode<T> Nearest(double[] position, out double distance)
        {
            ColorTreeNode<T> result = Root;
        ...
        /// 
        [Obsolete]
        public IList<ColorTreeNode<T>> GetNodesInsideRegion(Hyperrectangle region)
        {
            return getNodesInsideRegion(this.Root, region, region);
        }

        [Obsolete]
        private IList<ColorTreeNode<T>> getNodesInsideRegion(ColorTreeNode<T> node, Hyperrectangle region, Hyperrectangle subRegion)
        {
            var result = new List<ColorTreeNode<T>>();
        ...
        private static Hyperrectangle leftRect(Hyperrectangle hyperrect, ColorTreeNode<T> node)
        {
        ...
        private static Hyperrectangle rightRect(Hyperrectangle hyperrect, ColorTreeNode<T> node)
        {
            //var rect = hyperrect.ToRectangle();
            //return (node.Axis != 0 ?
        ...
        /// <summary>
        ///   Creates the Root node for a new <see cref="ColorTree{T}"/> given
        ...
        ///
        /// <returns>The Root node for a new <see cref="ColorTree{T}"/>
        ///   contained the given <paramref name="points"/>.</returns>
        ///
        protected static ColorTreeNode<T> CreateRoot(double[][] points, bool inPlace, out int leaves)
        {
            ...
            ColorTreeNode<T> Root = create(points, 0, dimensions, 0, points.Length, comparer, ref leaves);

            // Create and return the newly formed tree
            return Root;
        }

        private static ColorTreeNode<T> create(double[][] points,
        ...
        /// </summary>
        ///
        private void nearest(ColorTreeNode<T> current, double[] position,
        double radius, ICollection<NodeDistance<ColorTreeNode<T>>> list)
        {
            ...
            if (d <= radius)
                list.Add(new NodeDistance<ColorTreeNode<T>>(current, d));
        ...
        /// </summary>
        ///
        private void nearest(ColorTreeNode<T> current, double[] position, ColorTreeNodeCollection<T> list)
        {
        ...
        private void nearest(ColorTreeNode<T> current, double[] position, ref ColorTreeNode<T> match, ref double minDistance)
        {
            // Compute distance from this node to the point
            double d = this.Distance(position, current.Position);
        ...
        private bool approximate(ColorTreeNode<T> current, double[] position,
        ColorTreeNodeCollection<T> list, int maxLeaves, ref int visited)
        {
            // Compute distance from this node to the point
            double d = this.Distance(position, current.Position);
        ...
        private bool approximateNearest(ColorTreeNode<T> current, double[] position,
        ref ColorTreeNode<T> match, ref double minDistance, int maxLeaves, ref int visited)
        {
            // Compute distance from this node to the point
            double d = this.Distance(position, current.Position);
        ...
        /// <param name="position">A double-vector with the same number of elements as dimensions in the tree.</param>
        ///
        protected ColorTreeNode<T> AddNode(double[] position)
        {
            count++;
            var root = Root;
            ColorTreeNode<T> node = insert(ref root, position, 0);
            Root = root;
            return node;
        }

        private ColorTreeNode<T> insert(ref ColorTreeNode<T> node, double[] position, int depth)
        {
            if (node == null)
            {
                // Base case: node is null
                return node = new ColorTreeNode<T>()
                {
        ...
        ///
        public void CopyTo(ColorTreeNode<T>[] array, int arrayIndex)
        {
            foreach (var node in this)
            {
        ...
        /// </returns>
        /// 
        public virtual IEnumerator<ColorTreeNode<T>> GetEnumerator()
        {
            if (Root == null)
                yield break;

            var stack = new Stack<ColorTreeNode<T>>(new[] { Root });

            while (stack.Count != 0)
            {
                ColorTreeNode<T> current = stack.Pop();
