...
namespace PixelStacker.Logic.Extensions
{
    ...
    public static class ExtendColor
    {
        ...
        /// <summary>
        /// Custom color matching algorithm, gets the average color distance between an average color and an image
        /// </summary>
        /// <param name="c"></param>
        /// <returns></returns>
        public static int GetColorDistanceAveragePerPixel(this Color cAvg, Bitmap toMatch)
        {
            int distTotal = 0;
            int count = toMatch.Width * toMatch.Height;
            toMatch.ToViewStreamParallel(null, (x, y, c) => {
                int dist = c.GetColorDistance(cAvg);
                Interlocked.Add(ref distTotal, dist);
            });

            return distTotal / count;
        }
